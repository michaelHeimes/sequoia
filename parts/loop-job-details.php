<div class="list-wrap">
	<h4>Responsibilities</h4>
	<?php the_field('responsibilities');?>
</div>
<div class="apply-wrap">
	<h4>Apply</h4>
	<p>Send your resume to <a href="mailto:careers@sequioarg.com?subject=<?php the_title() ?>">careers@sequioarg.com</a></p>
</div>
<div class="job-share text-center">
	<button class="button outline">share</button>
	
	<div class="a2a_kit a2a_default_style grid-x align-center">
		<div class="inner">
			<a class="a2a_button_facebook" href="#">
				<svg xmlns="http://www.w3.org/2000/svg" width="6.361" height="12.72" viewBox="0 0 6.361 12.72">
				  <path id="Path_8" data-name="Path 8" d="M137.188,2.112h1.161V.09A15,15,0,0,0,136.658,0a2.686,2.686,0,0,0-2.821,2.989V4.77h-1.848V7.031h1.848V12.72H136.1V7.031h1.773l.281-2.261H136.1V3.213c0-.653.176-1.1,1.087-1.1Z" transform="translate(-131.989)" fill="#fff"/>
				</svg>
			</a>
			<a class="a2a_button_twitter" href="#">
				<svg xmlns="http://www.w3.org/2000/svg" width="13.543" height="11.004" viewBox="0 0 13.543 11.004">
				  <g id="Group_37" data-name="Group 37" transform="translate(0 0)">
					<path id="Path_131" data-name="Path 131" d="M13.543,49.3a5.789,5.789,0,0,1-1.6.438,2.761,2.761,0,0,0,1.221-1.535,5.549,5.549,0,0,1-1.761.672,2.776,2.776,0,0,0-4.8,1.9,2.859,2.859,0,0,0,.064.633,7.859,7.859,0,0,1-5.723-2.9A2.777,2.777,0,0,0,1.8,52.217a2.742,2.742,0,0,1-1.254-.342v.03a2.789,2.789,0,0,0,2.224,2.728,2.771,2.771,0,0,1-.728.091,2.455,2.455,0,0,1-.526-.047,2.8,2.8,0,0,0,2.594,1.934A5.579,5.579,0,0,1,.664,57.8,5.2,5.2,0,0,1,0,57.758,7.817,7.817,0,0,0,4.259,59a7.848,7.848,0,0,0,7.9-7.9c0-.123,0-.241-.01-.359A5.539,5.539,0,0,0,13.543,49.3Z" transform="translate(0 -48)" fill="#fff"/>
				  </g>
				</svg>
			</a>	    
			<a class="a2a_button_linkedin" data-url="<?php echo get_permalink();?>" href="#">
				<svg id="Group_20" data-name="Group 20" xmlns="http://www.w3.org/2000/svg" width="12.732" height="12.166" viewBox="0 0 12.732 12.166">
				  <path id="Path_5" data-name="Path 5" d="M13.232,7.962v4.7H10.509V8.281c0-1.1-.389-1.857-1.379-1.857a1.491,1.491,0,0,0-1.4.99,1.936,1.936,0,0,0-.088.672v4.58H4.9s.035-7.445,0-8.205H7.626V5.628c0,.018-.018.018-.018.035h.018V5.628a2.706,2.706,0,0,1,2.458-1.362c1.8,0,3.148,1.167,3.148,3.7ZM2.038.5A1.43,1.43,0,0,0,.5,1.915,1.409,1.409,0,0,0,2,3.329h.018A1.42,1.42,0,0,0,3.559,1.915,1.4,1.4,0,0,0,2.038.5ZM.659,12.666H3.382V4.461H.659Zm0,0" transform="translate(-0.5 -0.5)" fill="#fff"/>
				</svg>	
			</a>
			<a class="email" href="mailto:?subject=Job opening at Sequioa Realty Group - <?php the_title() ?>&body=Here's the link: <?php echo get_permalink();?>">
				<svg xmlns="http://www.w3.org/2000/svg" width="32.336" height="32.336" viewBox="0 0 32.336 32.336">
			  	<g id="Group_657" data-name="Group 657" transform="translate(-774.05 -3207.664)">
					<ellipse id="Ellipse_193" data-name="Ellipse 193" cx="16.168" cy="16.168" rx="16.168" ry="16.168" transform="translate(774.05 3207.664)" fill="#00733c"/>
					<path id="Path_578" data-name="Path 578" d="M121.574,134.4a1.178,1.178,0,0,0-.435.086l6.818,5.8a.663.663,0,0,0,.93,0l6.824-5.808a1.181,1.181,0,0,0-.428-.079Zm-1.163,1a1.22,1.22,0,0,0-.012.171v7.834a1.173,1.173,0,0,0,1.175,1.175h13.71a1.173,1.173,0,0,0,1.175-1.175v-7.834a1.2,1.2,0,0,0-.012-.171l-6.794,5.778a1.908,1.908,0,0,1-2.461,0Z" transform="translate(662.071 3084.34)" fill="#fff"/>
			  	</g>
				</svg>
			</a>
			<a class="copy-link" href="#" data-link="<?php echo get_permalink();?>">
				<svg xmlns="http://www.w3.org/2000/svg" width="32.336" height="32.336" viewBox="0 0 32.336 32.336">
			  	<g id="Group_659" data-name="Group 659" transform="translate(-774.05 -3207.664)">
					<g id="Group_657" data-name="Group 657">
				  	<ellipse id="Ellipse_193" data-name="Ellipse 193" cx="16.168" cy="16.168" rx="16.168" ry="16.168" transform="translate(774.05 3207.664)" fill="#00733c"/>
					</g>
					<g id="Group_658" data-name="Group 658" transform="translate(0.184 -1.043)">
				  	<path id="Path_578" data-name="Path 578" d="M89.238,203.419a3.758,3.758,0,0,1-.588.765l-3.608,3.608a3.825,3.825,0,0,1-5.41-5.41l2.582-2.582a5.1,5.1,0,0,0,.39,2.316l-1.617,1.619a1.913,1.913,0,0,0,2.7,2.706l3.606-3.611a1.913,1.913,0,0,0,.37-2.182,1.913,1.913,0,0,0-.887-.887c.02-.033,1.381-1.4,1.381-1.4a4.342,4.342,0,0,1,.492.408,4.147,4.147,0,0,1,.408.494,3.765,3.765,0,0,1,.578,1.224,3.818,3.818,0,0,1-.4,2.927Z" transform="translate(870.773 3021.634) rotate(45)" fill="#fff"/>
				  	<path id="Path_579" data-name="Path 579" d="M254.895,15.035l-2.579,2.582a5.14,5.14,0,0,0-.39-2.319l1.617-1.617a1.913,1.913,0,1,0-2.7-2.706l-3.608,3.606a1.919,1.919,0,0,0-.367,2.187,1.851,1.851,0,0,0,.367.519,1.9,1.9,0,0,0,.519.367c-.02.028-1.381,1.4-1.381,1.4a3.532,3.532,0,0,1-.489-.411,3.99,3.99,0,0,1-.411-.492,3.844,3.844,0,0,1,.413-4.918l3.6-3.606a3.825,3.825,0,0,1,5.41,5.41Z" transform="translate(626.486 3037.83) rotate(45)" fill="#fff"/>
					</g>
			  	</g>
				</svg>
			</a>	
		</div>
	
	</div>

</div>